...
class CanvasRepo(AccommodationRepo):
    ...
    async def list_submissions(self, *, course_id: int, quiz_id: int, engine: str) -> list[Submission]:
        ...
        print(f'submission pathL {path}')
        print(type(payload), payload[:1] if isinstance(payload, list) else payload)
    ...
    async def get_submission(self, *, course_id: int, quiz_id: int, user_id: int, engine: str) -> Optional[Submission]:
